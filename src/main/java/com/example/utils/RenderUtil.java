...
public class RenderUtil {
    // expandBySmallAmt so that the box doesnt look glitchy if showThroughWalls is enabled
    public static void renderBoundingBox(double _x, double _y, double _z, int rgb, float alphaMult, float partialTicks, boolean showThroughWalls, boolean expandBySmallAmt) {
        ...
        if (expandBySmallAmt) bb = bb.expand(0.01, 0.01, 0.01);

        if (showThroughWalls) GlStateManager.disableDepth();
        GlStateManager.disableCull();
        GlStateManager.disableTexture2D();
        drawFilledBoundingBox(bb, alphaMult, rgb);
...
//        Color c = new Color(SpecialColour.specialToChromaRGB(special), true);
    ...
    public static void renderBeaconBeam(
            double _x, double _y, double _z, int rgb, float alphaMult,
    ...
    public static void renderWayPoint(List<String> lines, double x, double y, double z, int color, float partialTicks) {
        ...
        double _x = x - viewerX + 0.5f;
        double _y = y - viewerY - viewer.getEyeHeight();
        double _z = z - viewerZ + 0.5f;

        double distSq = _x * _x + _y * _y + _z * _z;
    ...
    private static void renderNametag(List<String> lines, int color) {
        ...
        for (String str : lines) {
            ...
            worldrenderer.pos(-j - 1, -1 + i, 0.0D).color(red, green, blue, 0.25F).endVertex();
            worldrenderer.pos(-j - 1, 8 + i, 0.0D).color(red, green, blue, 0.25F).endVertex();
            worldrenderer.pos(j + 1, 8 + i, 0.0D).color(red, green, blue, 0.25F).endVertex();
            worldrenderer.pos(j + 1, -1 + i, 0.0D).color(red, green, blue, 0.25F).endVertex();
            tessellator.draw();
            GlStateManager.enableTexture2D();
...
//            }
            ...
            if (needsWrap)
            {
                ...
                // Wraps to left sometimes... (we dont want that, we want exact positioning
                /*if (mouseX > screenWidth / 2)
                {
                    tooltipX = mouseX - 16 - tooltipTextWidth;
                }
            ...
            if (tooltipY + tooltipHeight + 6 > screenHeight - 10 &&
                    (tooltipY - ((tooltipY + tooltipHeight + 6) / 8)) > 0) {
                tooltipY -= (tooltipY + tooltipHeight + 6) / 8;
            }
